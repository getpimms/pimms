...
export const LifetimeOffer = ({ tkey, showBusinessPlan = true }: { tkey: string; showBusinessPlan?: boolean }) => {
  const t = useTranslations(tkey);

  return (
    <Section className="py-12 sm:py-16 md:py-20" id="lifetime">
      <div className="space-y-10 md:space-y-12">
        {/* Header */}
        <div className="text-center space-y-3">
          <H2>{t("lifetime_offer.title")}</H2>
          <Paragraph className="text-base md:text-lg max-w-2xl mx-auto">{t("lifetime_offer.subtitle")}</Paragraph>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
          ...
          {/* Pro Plan */}
          <div className="bg-white rounded-lg border-2 border-[#3970ff] p-6 md:p-8 relative">
            <div className="absolute -top-2.5 left-6 md:left-8 bg-[#3970ff] text-white px-3 py-1 rounded text-xs font-semibold uppercase">
            ...
            <div className="space-y-5 md:space-y-6">
              ...
              <CtaButtonBig type="lifetime" size="lg" className="w-full" value={t("lifetime_offer.cta")} />
            </div>
          </div>
        </div>
        ...
        {/* Business Plan */}
        {showBusinessPlan && (
          <div className="max-w-5xl mx-auto bg-white rounded-lg border border-gray-200 p-6 md:p-10">
            ...
            <div className="border-t border-gray-200 pt-5 md:pt-6">
              ...
              <div className="grid md:grid-cols-2 gap-2.5 md:gap-3 mb-6 md:mb-8">
                {[
                  ...
                  t("lifetime_offer.feature_custom_reports")
                ].map((feature, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <Check className="w-4 h-4 md:w-5 md:h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span className="text-sm md:text-base text-[#08272E]">{feature}</span>
              ...
              <div className="text-center">
                <Button size="lg" className="w-full md:w-auto md:px-12" asChild>
                  <Link href={`${APP_URL}/register?plan=business`}>{t("lifetime_offer.get_started")}</Link>
                </Button>
              </div>
            </div>
          </div>
